...
REM Also means 'winget' and other .appx reliant software break
set /A break_windows_store=0

REM Can be reverted with 'sfc /scannow'; used to entirely disable Windows Defender
set /A delete_windows_security=1

REM Will break SteamVR Base Station support
...
REM Instructions on how to stay secure are located @ README.adoc
...
REM https://nvidia.custhelp.com/app/answers/detail/a_id/5157
REM https://nvidia.custhelp.com/app/answers/detail/a_id/5159/~/v-sync-off-not-recommended-as-a-global-setting-starting-with-driver-version
set /A enable_mpo=1

REM Install .NET Framework 2 and 3.5 for backwards compatibility
set /A install_dotnet_2_and_3=0

REM Ensure reliability no matter the (DOCSIS 3.1/ADSL/Fiber) networking conditions; great for both games and torrents
...
REM Disables Game Bar and all Xbox functionality; heavily reliant on each other
set /A remove_xbox=0

REM Disables mouse smoothing across all software & games
set /A run_markc_mousefix=1

REM TODO DESCRIPTION
set /A disable_netbios=1

REM Disable Windows Script Host (.vbs/.vbe/.ws/.wsh/.js/.jse); decreases attack surface, and without downsides (for some)
set /A z_disable_script_host=0

REM 1 if you are absolutely sure this PC is used solely for games
set /A z_disable_tcp_nagle=0
...
echo z_disable_script_host = %z_disable_script_host%
echo z_disable_tcp_nagle = %z_disable_tcp_nagle%
echo.
Pause
...
REM Activation is required for some changes; requires ClipSVC to fully work
start /high /b "" "%~dp0\MAS_1.4\KMS38_Activation.cmd"

REM Prefer to disable "Full Screen Optimizations"; FSO uses more GPU, lowering FPS in GPU-intensive games
...
REM >> [GROUP 1] File Manager & Windows Shell tweaks <<
...
REM Performance Options -> Visual Effects -> Custom
...
REM Set the "blue" Solid colour Background; using a wallpaper keeps an image loaded into memory
...
REM Disable JPEG wallpaper quality reduction
reg.exe add "HKCU\Control Panel\Desktop" /v "JPEGImportQuality" /t REG_DWORD /d "100" /f

REM >> [GROUP 1 END] <<


REM >> [GROUP 2] Privacy enhancers (ask Microsoft nicely to turn off data collectors, which waste system resources) <<
...
REM Disable Sharing of handwriting data
reg.exe add "HKLM\SOFTWARE\Policies\Microsoft\Windows\TabletPC" /v "PreventHandwritingDataSharing" /t REG_DWORD /d 1 /f
REM Remove Search/Cortana from taskbar
reg.exe add "HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\Search" /v "SearchboxTaskbarMode" /t REG_DWORD /d 0 /f
REM Remove People buttom from taskbar
...
REM Disable transmission of typing information
reg.exe add "HKCU\SOFTWARE\Microsoft\Input\TIPC" /v "Enabled" /t REG_DWORD /d 0 /f
REM Disable Microsoft conducting experiments with this machine
...
REM Disable syncing of text messages to Microsoft
reg.exe add "HKLM\SOFTWARE\Policies\Microsoft\Windows\Messaging" /v "AllowMessageSync" /t REG_DWORD /d 0 /f
REM Disable application access to user account information
...
REM Disable application access of diagnostic information
...
REM Disable synchronization of all settings to Microsoft
...
REM No web search through Start Menu
...
REM >> [GROUP 3] Disable legacy features to increase security <<

REM Disable SMBv1, it's insecure and slow compared to SMBv3
...
REM Disable updates for Speech Recognition and Speech Synthesis
...
REM Disable peer-to-peer functionality in Windows Update
...
REM Disable feedback reminders
...
REM Disable Sticky Keys, Toggle Keys, and Filter Keys
reg.exe add "HKCU\Control Panel\Accessibility\StickyKeys" /v "Flags" /t REG_SZ /d 50 /f
reg.exe add "HKCU\Control Panel\Accessibility\ToggleKeys" /v "Flags" /t REG_SZ /d 58 /f
reg.exe add "HKCU\Control Panel\Accessibility\Keyboard Response" /v "Flags" /t REG_SZ /d 122 /f

REM Never use non-stock configurations with Insider builds (specially Fast Ring)
...
if %delete_windows_security%==1 (
	...
	takeown.exe /d Y /s %computername% /u %username% /f "%WinDir%\System32\smartscreen.exe"
	icacls.exe "%WinDir%\System32\smartscreen.exe" /grant:r %username%:F
	taskkill.exe /im smartscreen.exe /f
	del "%WinDir%\System32\smartscreen.exe" /s /f /q
	takeown.exe /d Y /s %computername% /u %username% /f "%WinDir%\System32\SecurityHealthSystray.exe"
	...
	takeown.exe /d Y /s %computername% /u %username% /f "%WinDir%\System32\SecurityHealthHost.exe"
	...
	takeown /d Y /s %computername% /u %username% /f "C:\Program Files\Windows Defender" /R
	...
	REM reg.exe add "HKLM\SYSTEM\CurrentControlSet\Services\MpsSvc" /v "Start" /t REG_DWORD /d 4 /f
...
if %network_adapter_tweaks%==1 (
	...
	REM PowerShell's "Set-NetTCPSetting" doesn't allow changing property "CongestionProvider"
	netsh.exe int tcp set supplemental Internet congestionprovider=ctcp
	REM Don't allow the Multimedia Class Scheduler Service to throttle
	reg.exe add "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Multimedia\SystemProfile" /v "NetworkThrottlingIndex" /t REG_DWORD /d 4294967295 /f
	reg.exe add "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Multimedia\SystemProfile" /v "SystemResponsiveness" /t REG_DWORD /d 0 /f

	REM Android & Linux's default TTL; disregard packets quicker if they haven't reached their target destination
	reg.exe add "HKLM\SYSTEM\CurrentControlSet\Services\Tcpip\Parameters" /v "DefaultTTL" /t REG_DWORD /d 64 /f
	REM Half port re-use time since TTL is halved from its default value
	...
	REM Never apply QoS/throttle on this PC, only allow the router and/or modem to do so
...
REM Disable UAC: EnableLUA at 0 will break startup of some software, such as https://eddie.website/
reg.exe add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System" /v "EnableLUA" /t REG_DWORD /d 1 /f
reg.exe add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System" /v "ConsentPromptBehaviorAdmin" /t REG_DWORD /d 0 /f

REM Never expire Windows' login password
net accounts /maxpwage:unlimited
REM Unhide lots of Power Plan options
...
REM Automated file cleanup (without user interaction) is a bad idea
...
REM A worthless security measure, just use disk encryption
reg.exe add "HKLM\SYSTEM\CurrentControlSet\Control\Session Manager\Memory Management" /v ClearPageFileAtShutdown /t REG_DWORD /d 0 /f
REM After downloading a file, assume it's also going to be ran; don't block it from running
...
REM https://winaero.com/disable-timeline-windows-10-group-policy/
reg.exe add "HKLM\SOFTWARE\Policies\Microsoft\Windows\System" /v "EnableActivityFeed" /t REG_DWORD /d 0 /f
REM Disable Windows Firewall, since Windows Filtering Platform (WFP) is better
netsh.exe advfirewall set allprofiles state off

REM Aero Shake is a bad feature
reg.exe add "HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Advanced" /v "DisallowShaking" /t REG_DWORD /d 1 /f

REM Use old battery flyout, makes it more difficult to accidentally screw up the power options
reg.exe add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\ImmersiveShell" /v "UseWin32BatteryFlyout" /t REG_DWORD /d 1 /f
REM Use old volume flyout, makes it easier to quickly change advanced settings of the current audio device
reg.exe add "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\MTCUVC" /v "EnableMtcUvc" /t REG_DWORD /d 0 /f
REM Enable additional BSOD details
reg.exe add "HKLM\SYSTEM\CurrentControlSet\Control\CrashControl" /v "DisplayParameters" /t REG_DWORD /d 1 /f
REM Disable all Content Delivery Manager features
...
REM For a mouse, extra window top-bar border width is counter intuitive
reg.exe add "HKCU\Control Panel\Desktop\WindowMetrics" /v "PaddedBorderWidth" /t REG_SZ /d 0 /f
REM Ensure audio ducking/audio attenuation is disabled
...
if %remove_xbox%==1 (
	...
	takeown /d Y /f "%WinDir%\System32\GameBarPresenceWriter.exe" /a
...
REM Turn off Game DVR; gets rid of "You'll need a new app to open this ms-gamingoverlay" on LTSC 2019
...
REM TODO: Move this somewhere else
...
REM Works without issue these days on 1809 (LTSC 2019) or newer
...
REM Don't download Microsoft's Malicious Removal Tool automatically
reg.exe add "HKLM\SOFTWARE\Policies\Microsoft\MRT" /v "DontOfferThroughWUAU" /t REG_DWORD /d 1 /f
REM Disable Start Menu's Live Tiles
...
REM Hide News and Interests icon on Windows 10 taskbar
reg.exe add "HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\Feeds" /v "ShellFeedsTaskbarViewMode" /t REG_DWORD /d 2 /f
REM Disable first sign-in animation (initiates on a new user account)
...
:FontCache
sc stop "FontCache"
REM sc config "FontCache" start=disabled
